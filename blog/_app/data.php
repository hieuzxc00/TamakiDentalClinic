...
<?php
	...
	foreach($matches[1] as $index=>$params){
		if(preg_match('@^\s@',$params)||!strlen($params)){
			...
			if(strlen(@$param_output['category_exclude'])){
        		$category_exclude=$param_output['category_exclude'];
			}else{
				$category_exclude="''";
			}
...
function contribute_search( $category='', $field='', $keyword='', $order='', $sort='', $category_order='', $category_exclude='' ) {
	...
	foreach($contribute_index as $index=>$data){
		if( ($category && $data['category']!=$category) || ($field && $data['field']!=$field) || $category_exclude == $data['category']){
